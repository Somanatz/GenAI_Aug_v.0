...
import type { Event as EventInterface, StudentTask as TaskInterface } from '@/interfaces';
...
export default function StudentCalendarPage() {
  ...
  const onTaskSubmit = async (data: TaskFormValues) => {
    ...
    try {
      if (editingTask) {
        const updatedTask = await api.patch<TaskInterface>(`/student-tasks/${editingTask.id}/`, payload);
        setTasks(prevTasks => prevTasks.map(t => t.id === editingTask.id ? updatedTask : t));
        toast({ title: "Task Updated" });
      } else {
        const newTask = await api.post<TaskInterface>(`/student-tasks/`, payload);
  ...
  const handleToggleTaskCompletion = async (task: TaskInterface) => {
    try {
      const updatedTask = await api.patch<TaskInterface>(`/student-tasks/${task.id}/`, { completed: !task.completed });
  ...
  useEffect(() => {
      // Closes the date detail dialog if all items for that day have been removed.
  ...
  const DayContent = useCallback(({ date }: { date: Date }) => {
    ...
    const formatCustomTime = (isoDate: string) => {
        try {
            const d = parseISO(isoDate);
            return format(d, 'p').replace(' AM', 'a').replace(' PM', 'p').toLowerCase();
        } catch {
            return '';
        }
    };
    ...
    return (
        <div className="flex flex-col h-full overflow-hidden">
            <span className={cn(
                "self-end p-1 text-sm font-medium",
                dayIsToday && "bg-blue-600 text-white rounded-full h-7 w-7 flex items-center justify-center m-1"
            )}>
                {format(date, 'd')}
            </span>
            <div className="flex-grow space-y-0.5 p-0.5">
                {items.map(item => (
                    <div
                        ...
                        className="bg-blue-500 text-white rounded-sm text-xs px-1.5 py-0.5 truncate cursor-pointer hover:bg-blue-600"
                        onClick={(e) => {
                            e.stopPropagation();
                            setSelectedDate(date);
                            setIsDateDialogOpen(true);
                    ...
                    >
                        <span className="font-semibold">
                            {item.type === 'event' && item.date ? `${formatCustomTime(item.date)} ` : ''}
                        </span>
                        {item.title}
                    </div>
                ))}
    ...
    <div className="space-y-8 p-4 md:p-6 flex flex-col h-[calc(100vh-8rem)]">
      <header>
        <h1 className="text-3xl font-bold flex items-center">
          <CalendarDays className="mr-3 h-8 w-8 text-primary" /> My Calendar
        </h1>
        <p className="text-muted-foreground mt-1">
          A unified view of your school events and personal tasks.
        </p>
      </header>
      
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <Card><CardHeader className="flex-row items-center justify-between pb-2"><CardTitle className="text-sm font-medium">Total School Events</CardTitle><School className="h-4 w-4 text-muted-foreground"/></CardHeader><CardContent><div className="text-2xl font-bold">{isLoading ? <Skeleton className="h-8 w-12"/> : events.length}</div></CardContent></Card>
        <Card><CardHeader className="flex-row items-center justify-between pb-2"><CardTitle className="text-sm font-medium">Pending Personal Tasks</CardTitle><ListTodo className="h-4 w-4 text-muted-foreground"/></CardHeader><CardContent><div className="text-2xl font-bold">{isLoading ? <Skeleton className="h-8 w-12"/> : pendingTasks.length}</div></CardContent></Card>
        <Card className="md:col-span-2 lg:col-span-1"><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Scheduled for Today</CardTitle></CardHeader><CardContent>
            {todayItems.length > 0 ? (
                <ul className="text-sm text-muted-foreground space-y-1">
                    {todayItems.map(item => <li key={`${item.type}-${item.id}`} className="flex items-center gap-2"><div className={cn("h-2 w-2 rounded-full", item.type === 'event' ? 'bg-blue-500' : 'bg-green-500')}></div>{item.title}</li>)}
                </ul>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start flex-grow">
               ...
               error ? <p className="text-destructive p-4">{error}</p> : (
                <Calendar
                  ...
                  classNames={{
                    ...
                    month: "space-y-4 flex-grow flex flex-col",
                    caption: "flex justify-center pt-1 relative items-center",
                    caption_label: "text-lg font-medium",
                    nav: "space-x-1 flex items-center",
                    nav_button: "h-8 w-8 bg-transparent p-0 opacity-50 hover:opacity-100",
                    table: "w-full border-collapse flex flex-col flex-grow",
                    head_row: "flex divide-x divide-gray-200 dark:divide-neutral-800 border-b border-gray-200 dark:border-neutral-800",
                    head_cell: "text-muted-foreground flex-1 font-normal text-[0.8rem] text-center p-2",
                    row: "flex w-full divide-x divide-gray-200 dark:divide-neutral-800 flex-1",
                    cell: "text-left text-sm p-0 relative flex-1 focus-within:relative focus-within:z-20 border-b border-gray-200 dark:border-neutral-800",
                    day: "h-full w-full p-0 font-normal",
                    day_selected: "bg-yellow-100 text-accent-foreground dark:bg-yellow-800/50",
                    day_today: "",
                    day_outside: "text-muted-foreground opacity-50 bg-gray-50 dark:bg-background/40",
        ...
        <div className="lg:col-span-1 space-y-6">
          <Card>
              <CardHeader>
                  <CardTitle className="flex items-center text-lg"><ListTodo className="mr-2 h-5 w-5"/>Active Tasks</CardTitle>
              </CardHeader>
              <CardContent>
                        ...
                        {pendingTasks.map(task => (
                            <li key={`pending-${task.id}`} className="flex items-start gap-3 text-sm">
                              ...
                              <div>
                                ...
                                <p className="text-xs text-muted-foreground">{format(parseISO(task.due_date), 'E, MMM d')}</p>
                              </div>
                            </li>
                        ))}
                      </ul>
              ...
              {tasks.filter(t => t.completed).length > 0 ? (
                   <ul className="space-y-3 max-h-48 overflow-y-auto pr-2">
                      {tasks.filter(t => t.completed).map(task => (
                          <li key={`done-${task.id}`} className="flex items-center gap-3 text-sm text-muted-foreground">
      ...
      <Dialog open={isDateDialogOpen} onOpenChange={setIsDateDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Items for {selectedDate ? format(selectedDate, 'PPP') : 'Date'}</DialogTitle>
            <DialogDescription>Manage your tasks and view events for this day.</DialogDescription>
          </DialogHeader>
            ...
            {itemsForSelectedDate.length > 0 ? itemsForSelectedDate.map(item => (
              <div key={`${item.type}-${item.id}`} className={cn("p-3 border-l-4 rounded-r-md flex justify-between items-center", item.type === 'event' ? 'bg-blue-50 border-blue-400 dark:bg-blue-900/50 dark:border-blue-600' : 'bg-green-50 border-green-400 dark:bg-green-900/50 dark:border-green-600')}>
                <div>
                    ...
                    <p className="text-xs text-muted-foreground">{item.type === 'event' ? 'School Event' : 'Personal Task'}</p>
                </div>
                {item.type === 'task' && (
                  <div className="flex items-center gap-1">
                    <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => handleEditClick(item as TaskInterface)}><Edit className="h-4 w-4"/></Button>
                    <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:text-destructive" onClick={() => handleDeleteTask(item.id)}><Trash2 className="h-4 w-4"/></Button>
                  </div>
                )}
              </div>
          ...
          <DialogFooter>
               <Button variant="outline" onClick={() => setIsDateDialogOpen(false)}>Close</Button>
               <Button onClick={() => handleAddNewClick(selectedDate)}><PlusCircle className="mr-2 h-4 w-4"/> Add New Task</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
          ...
          <Form {...taskForm}>
            <form onSubmit={taskForm.handleSubmit(onTaskSubmit)} className="space-y-4">
              ...
              <DialogFooter>
                ...
                <Button type="submit" disabled={isSubmitting}>{isSubmitting ? <Loader2 className="animate-spin"/> : 'Save Task'}</Button>
              </DialogFooter>
            </form>
          </Form>
        </DialogContent>
